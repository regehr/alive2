...
#include "Target/RISCV/RISCVGenSubtargetInfo.inc"
...
void riscv2llvm::lift(MCInst &I) {
  ...
  if (uncompressInst(uncompressedInst, I, *STI)) {
    // Overwrite the original MCInst as CurInst is also used by helper
    // functions.
    I = uncompressedInst;
  }
  ...
  // JAL is a direct jump when its link address is discarded.
  // C_J also represents the synthetic branch generated by checkEntryBlock.
  case RISCV::JAL:
  case RISCV::C_J: {
    /*
    ...
    int operand = 0;
    if (opcode == RISCV::JAL) {
      if (CurInst->getOperand(0).getReg() != RISCV::X0) {
        *out << "\nERROR: JAL with a nonzero destination register is not "
                "supported\n";
        exit(-1);
      }
      operand = 1;
