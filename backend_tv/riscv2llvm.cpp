...
/**
 * TODO: Possibly refactor into general function in mc2llvm,
 * Also could combine sdiv and udiv
 */
void riscv2llvm::lift_udiv(uint64_t size) {
  ...
  auto LHS = readFromRegOperand(1, intTy);
  auto RHS = readFromRegOperand(2, intTy);
...
void riscv2llvm::lift_sdiv(uint64_t size) {
  ...
  auto RHSIsAllOnes = createICmp(ICmpInst::Predicate::ICMP_EQ, RHS, allOnes);
  auto isOverflow = createAnd(LHSIsIntMin, RHSIsAllOnes);
  createBranch(isOverflow, overflowBB, divBB);

  LLVMBB = overflowBB;
  createStore(intMin, resPtr);
  createBranch(retBB);
...
void riscv2llvm::lift_urem(uint64_t size) {
  ...
  auto resPtr = createAlloca(intTy, getUnsignedIntConst(1, 64), "");
  createStore(LHS, resPtr);
  auto RHSIsZero = createICmp(ICmpInst::Predicate::ICMP_EQ, RHS, zero);
  createBranch(RHSIsZero, retBB, remBB);

  LLVMBB = remBB;
  auto remResult = createURem(LHS, RHS);
...
void riscv2llvm::lift_srem(uint64_t size) {
  ...
  LLVMBB = overflowBB;
  createStore(zero, resPtr);
  createBranch(retBB);

  LLVMBB = remBB;
  auto remResult = createSRem(LHS, RHS);
  createStore(remResult, resPtr);
  createBranch(retBB);
